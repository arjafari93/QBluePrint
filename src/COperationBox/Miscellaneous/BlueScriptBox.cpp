...
void CBlueScriptBox::evaluateOperation()
{
    ...
    if (outputList.length() > m_listOfOutputTerminals.length())
    { // TODO: show warning to user
        setBlueBox_warningMsg("JS function is supposed to return an array of " + QString::number(m_listOfOutputTerminals.length()) + " elements, but it returns " +
                              QString::number(outputList.length()) + " elements");
        return;
    }
...
const QString& CBlueScriptBox::scriptFunctionBody() const { return m_scriptFunctionBody; }
...
int CBlueScriptBox::numInputs() const { return m_numInputs; }
...
int CBlueScriptBox::numOutputs() const { return m_numOutputs; }
