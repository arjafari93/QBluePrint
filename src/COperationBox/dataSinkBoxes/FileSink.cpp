...
CFileSink::CFileSink(int newBlueBox_xPos, int newBlueBox_yPos, QObject* parent) : COperationBox{m_uniqueBoxName, newBlueBox_xPos, newBlueBox_yPos, blueBoxWidth, blueBoxHeight, parent}
{
    ...
    m_pathToFile = QCoreApplication::applicationDirPath() + "/fileSinkOutputs/" + QString::number(QDateTime::currentMSecsSinceEpoch()) + ".txt";
...
CFileSink::~CFileSink() { flushStringsIntoFile(); }
...
void CFileSink::flushStringsIntoFile()
{
    ...
    if (!fileInstance.open(QIODevice::Append | QIODevice::Text))
    { // open in apppend mode
