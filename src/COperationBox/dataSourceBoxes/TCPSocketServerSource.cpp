    ...
    : COperationBox{m_uniqueBoxName, newBlueBox_xPos, newBlueBox_yPos, blueBoxWidth + 80, blueBoxHeight + 30, parent}, mp_tcpServer{nullptr}
{
    ...
    if (!mp_tcpServer->listen(QHostAddress::Any, 0))
    { // Pass 0 to get a random available port
...
int CTCPSocketServerSource::serverPortNum() const { return m_serverPortNum; }
...
void CTCPSocketServerSource::setServerPortNum(int newServerPortNum)
{
    ...
    if (mp_tcpServer && !mp_tcpServer->listen(QHostAddress::Any, newServerPortNum))
    { // Start on a new port
