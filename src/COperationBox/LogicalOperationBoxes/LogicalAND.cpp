...
class CLogicalANDVisitor : public CValueVisitor
{
  public:
    explicit CLogicalANDVisitor(CRawValueBase* rhs) : mp_rhs(rhs) {}
...
CLogicalAND::CLogicalAND(int newBlueBox_xPos, int newBlueBox_yPos, QObject* parent) : COperationBox{m_uniqueBoxName, newBlueBox_xPos, newBlueBox_yPos, blueBoxWidth, blueBoxHeight, parent}
{
    m_blueBox_GUIType = CBPStatic::EBPDelegateGUIType::E_BigTextOperator;
    m_blueBox_HeadColor = QColor(155, 190, 200);
...
void CLogicalAND::evaluateOperation()
{
    ...
    if (!lhs || !rhs)
        return;
    CLogicalANDVisitor visitor(rhs);
    lhs->accept(visitor);
