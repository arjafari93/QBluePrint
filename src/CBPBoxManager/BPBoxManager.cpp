...
QQmlListProperty<COperationBox> CBPBoxManager::listOfBlueBoxes()
{
...
QQmlListProperty<CFlowConnectionLine> CBPBoxManager::listOfFlowConnectionLines()
{
...
void CBPBoxManager::lineFlowStartedFromConnectionPoint(  QObject * startTerminal)
{
...
/*!
 * \brief CBPBoxManager::lineFlowPosChagnedFromConnectionPoint
 * xpos and ypos are differential and they dont show absolute position int he screen
 * \param posX
 * \param posY
 */
void CBPBoxManager::lineFlowPosChagnedFromConnectionPoint( const int & relPosX , const int & relPosY, const int & absPosX , const int & absPosY )
{
...
/*!
 * \brief CBPBoxManager::lineFlowReleased
 * xpos and ypos are differential and they dont show absolute position int he screen
 * \param relPosX
 * \param relPosY
 * \param absPosX
 * \param absPosY
 */
void CBPBoxManager::lineFlowReleased(  const int & absPosX , const int & absPosY )
{
...
void CBPBoxManager::removeLineFromListModel(CFlowConnectionLine *lineToBeRemoved)
{
...
void CBPBoxManager::removeBPBoxFromListModel(QObject *boxToBeRemoved)
{
...
bool CBPBoxManager::createNewBoxFromGivenType( const QString &boxName , const int &posX, const int &posY )
{
    try{
...
void CBPBoxManager::setApplicationScaleFactor(const float  &newValue)
{
...
COperationBox *CBPBoxManager::getBPBoxFromUniqieID(const int &BPBoxID)
{
...
void CBPBoxManager::setDarkThemeEnabled(bool newDarkThemeEnabled)
{
    if (m_darkThemeEnabled == newDarkThemeEnabled)
        return;
    m_darkThemeEnabled = newDarkThemeEnabled;
...
void CBPBoxManager::setIsArchiveSearchActive(bool newIsArchiveSearchActive)
{
    if (m_isArchiveSearchActive == newIsArchiveSearchActive)
        return;
    m_isArchiveSearchActive = newIsArchiveSearchActive;
    emit isArchiveSearchActiveChanged();
}

const QString &CBPBoxManager::currentBluePrintFilePath() const
{
    return m_currentBluePrintFilePath;
}

void CBPBoxManager::setCurrentBluePrintFilePath(const QString &newCurrentBluePrintFilePath)
{
...
COperationBox * CBPBoxManager::checkIntersectionsWithBoxes(const int &absPosX, const int &absPosY)
{
...
CIOTerminal *  CBPBoxManager::checkIntersectionsWithTerminals( COperationBox *boxToBeSearched, const int &absPosX, const int &absPosY)
{
...
void CBPBoxManager::recalculateMaxPositions()
{
    for(auto currentBox : m_listOfBlueBoxes ){
...
bool CBPBoxManager::saveBluePrintInfo(const QString & pathToFile)
{
...
bool CBPBoxManager::loadBluePrintInfo(const QString &pathToFile)
{
    ...
    for (const QJsonValue& flowLineValue : flowLinesArray)
    {
        ...
        auto pStartBPBox = getBPBoxFromUniqieID( flowLineObject["startBoxID"].toString().toInt() );
        ASSERTWITHRETURN(pStartBPBox , false );
        auto pEndBPBox = getBPBoxFromUniqieID( flowLineObject["endBoxID"].toString().toInt() );
...
void CBPBoxManager::clearCurrentBluePrintSession()
{
    // lets clear current info if any
